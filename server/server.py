...
class myServer(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        global board
        pixels = getsizeof(json.dumps(board.getPixels(state)))
        encoded = getsizeof(json.dumps(board.getPixels(state)).encode(FORMAT))
...
def main():
    global board
    board = WhiteBoard()
    httpd = HTTPServer(("", int(PORT)), myServer)
